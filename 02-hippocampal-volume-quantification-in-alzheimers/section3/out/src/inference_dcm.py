...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    print(f"Original Volume: {orig_vol.shape} Predicted Volume: {pred_vol.shape}\n")

    slice_idx = [int(orig_vol.shape[0]/4), int(orig_vol.shape[0]/2), int(orig_vol.shape[0]*3/4)]
    ...
    for idx, i in enumerate(slice_idx):
        ...
        slice_lbl = pred_vol[i,:orig_vol.shape[1],:orig_vol.shape[2]]
        
        # lbl_img = np.zeros((orig_vol.shape[2],orig_vol.shape[1],3))
        # lbl_img[:,:,0] = np.flip((slice_lbl >= 1)*0xff).T.astype(np.uint8)
        lbl_img = np.flip((slice_lbl >= 1)*0xff).T.astype(np.uint8)
        
        # This is how you create a PIL image from numpy array
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((290,290))
        pil_j = ImageOps.colorize(Image.fromarray(lbl_img, mode="L"), black ="black", white ="red").convert("RGBA").resize((290,290))
        
        # Paste the PIL image into our main report image object (pimg)
        pimg.paste(Image.blend(pil_i, pil_j, 0.5), box=(10 + 300*idx, 400))
        pimg.paste(Image.blend(pil_i, pil_j, 0.5), box=(10 + 300*idx, 400))
        pimg.paste(Image.blend(pil_i, pil_j, 0.5), box=(10 + 300*idx, 400))
        # pimg.paste(pil_j, box=(10, 400))
...
def get_series_for_inference(path):
    ...
    # DONE: create a series_for_inference variable that will contain a list of only 
    ...
    for i in dicoms:
        ...
        # print(i.SeriesDescription)
        # print(i.SeriesDate)
        if 'hippocrop' in i.SeriesDescription.lower():
...
if __name__ == "__main__":
    ...
    os_command(f"storescu 127.0.0.1 4242 -v -aec ORTHANC +r +sd {report_save_path}")
