...
from medpy.features.utilities import normalize
...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        # print(f"Before normalisation = {image.min()}, {image.max()}")
        pxl_min, pxl_max = image.min(), image.max()
        image = (image - pxl_min) / (pxl_max - pxl_min)
        # print(f"After normalisation = {image.min()}, {image.max()}")
        ...
        # print(f"Label before type {label.dtype}")
        ...
        # print(f"Label after type {label.dtype} min={label.min()} max={label.max()}")

        # TASK: Why do we need to cast label to int?
        # ANSWER: To convert label dtype from uint8 to int64. 
        # The label indicate class so using an integer makes sense. However, perhaps, having int with higher bits 
        # will make sure the data type during back propagation can can be large numbers and be negative
